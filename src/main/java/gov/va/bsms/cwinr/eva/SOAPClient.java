...
public class SOAPClient {
	...
	private boolean useBGS() {
		return (config.isValid("bgs-url", "-node"));
	}

	public SOAPClient(JavaService service) {
		javaService = service;
		if (!useBGS()) {
			LOGGER.info("Empty bgs-url supplied in eVA.config file. No bgs server available.");
		}
	}
		...
		try {
			...
			while (scan.hasNextLine()) {
				...
				for (int i = 0; i < f.length; i++)
					if ("''".equals(f[i]))
						f[i] = ""; // two single quotes is the empty string
