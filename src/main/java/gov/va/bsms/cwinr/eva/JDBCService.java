...
public class JDBCService {
	...
	static Logger LOGGER = LoggerFactory.getLogger(JDBCService.class);
	...
	public JDBCService(JavaService service) {
		...
		if (jdbc_url.length() < 2) {
			LOGGER.info("Empty jdbc-url supplied in eVA.config file. No jdbc server available.");
			return;
		}
		...
		try {
			...
			// jdbc:oracle:thin:@//<host>:<port>/<service_name> see
			// https://razorsql.com/articles/oracle_jdbc_connect.html
			conn = DriverManager.getConnection(jdbc_url, config.getString("jdbc-user", "-node"),
					config.getString("jdbc-password", "-node"));
			LOGGER.info("Connected With Oracle is " + (conn != null));
			...
			while (resultSet.next()) {
				...
				LOGGER.info("JDBC read " + note.toString());
				...
				// if (note.error != null) { throw new SQLException("JavaService Reported
				// Error"); } DELETE THIS
			}
	 ...
	 * gov.va.eva.JDBCService.insertCaseNote( int )
	 */
	public static void insertCaseNote(int caseDcmntId) throws SQLException {
		LOGGER.info("INSERTCASENOTE PROCEDURE HAS BEEN WAS CALLED caseDcmntId = " + caseDcmntId);
		CaseNote note = new CaseNote(String.valueOf(caseDcmntId), "TYPE", "4000", "4000",
				"dcmntTxt for gov.va.eva.JDBCService.insertCaseNote( int ) ");
	...
	public void call_balance() throws SQLException {
		...
		LOGGER.info("About to prepareCall " + pro);
		CallableStatement statement = conn.prepareCall(pro);
		LOGGER.info("After prepareCall " + pro);
		statement.registerOutParameter(1, Types.FLOAT);
		LOGGER.info("About to setInt() ");
		statement.setInt(2, 10000);
		LOGGER.info("About to executeUpdate() ");
		statement.executeUpdate();
		LOGGER.info("About to getFloat() ");
		float value = statement.getFloat(1);
		LOGGER.info("jdbc-procedure returned the value {}", value);
	}
}

// Example from
// https://examples.javacodegeeks.com/core-java/sql/jdbc-oracle-thin-driver-example/
